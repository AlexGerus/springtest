...
@Service
public class VideoJPAService {
    private final VideoRepository repository;

    public VideoJPAService(VideoRepository repository) {
    ...
    @PostConstruct
    void initDatabase() {
        repository.save(new VideoEntity("user", "Need HELP with your SPRING BOOT 3 App?",
                "SPRING BOOT 3 will only speed things up and make it super SIMPLE to serve templates and raw data."));
        repository.save(new VideoEntity("user", "Don't do THIS to your own CODE!",
                "As a pro developer, never ever EVER do this to your code. Because you'll ultimately be doing it to YOURSELF!"));
        repository.save(new VideoEntity("user", "SECRETS to fix BROKEN CODE!",
